...
async function createStructure(structure, currentPath = "") {
    for (const [key, value] of Object.entries(structure)) {
        ...
        if (orderItems.length > 0) {
            ...
            console.log(`Created _order.yaml in ${path}`);
        }
...
async function findFile(ref, sourceDir = SOURCE_DIR) {
    // First try a direct match in the expected location
    const possiblePaths = [
        join(sourceDir, 'integrations', `${ref}.md`),
        join(sourceDir, 'connections', `${ref}.md`),
        join(sourceDir, 'actions', `${ref}.md`),
        join(sourceDir, 'scenarios', `${ref}.md`),
        join(sourceDir, 'customers', `${ref}.md`),
        join(sourceDir, 'external-events', `${ref}.md`),
        join(sourceDir, 'actioninstancebyid', `${ref}.md`),
        join(sourceDir, 'field-mappings', `${ref}.md`),
        join(sourceDir, 'externalapilogs', `${ref}.md`),
        join(sourceDir, 'app-events', `${ref}.md`)
    ];
            ...
            if ('ref' in value) {
                ...
                if (ref === 'proxy-api') {
                    ...
                    if (existsSync(sourceFile)) {
                        await copyFile(sourceFile, join(path + '.md'));
                        console.log(`Copied ${sourceFile} to ${join(path + '.md')}`);
                    }
                } else {
                    ...
                    if (sourceFile) {
                        ...
                        await ensureDir(parentPath); // Make sure the parent directory exists
                        const targetFile = join(parentPath, filename);
